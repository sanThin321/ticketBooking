...
export const updateMember = async (req, res) => {
    try {
      ...
      const updatedMember = await RegisterMember.findById(member._id).populate('agencyId', 'agencyName');
...
export const getAllMembers = async (req, res) => {
    try {
      ...
      if (members.length === 0) {
        return res.status(404).json({ message: "No members found for this agency" });
      }
