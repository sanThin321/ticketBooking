...
export const PurchaseHistory = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`http://localhost:4004/pelrizhabtho/agency/ticket/${userId}`);
        if (res.status === 200) {
          setTickets(res.data);
        }
      } catch (error) {
  ...
  const downloadTicket = (ticket) => {
    ...
    doc.text(`Departure Time: ${new Date(ticket.departureTime).toLocaleString()}`, 20, 60);
    doc.text(`Arrival Time: ${new Date(ticket.arrivalTime).toLocaleString()}`, 20, 70);
                ...
                <div>
                  <div className="d-flex align-items-center gap-3 pe-5" style={{ borderRight: "1.5px solid #8DD3BB" }}>
                    ...
                    <div>
                      <p>{ticket.to}</p>
                      <h6>
                        <strong>{new Date(ticket.arrivalTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</strong>
                      </h6>
                    </div>
                  </div>
                </div>
